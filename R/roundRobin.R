...
#'
#' @format A character string vector with 18 team names
#'
"AFLW"
...
#' Arrange the components of an object in random order
#'
#' Simple utility to permute the components of an object
#'
#' @param object Any object with ordered components
#' @param ... Additional arguments to be passed on to methods
#'
#' @return A similar object with the components arranged in random
#'         order, but with the names and other attributes held fixed.
#' @export
#' @examples
#' Season_2021 <- randomise(round_robin(AFLW))
#' summary(Season_2021, "travel")
#'
...
#' @rdname randomize
#' @export
randomise <- function(object, ...) {
  UseMethod("randomize")
}

#' @rdname randomize
#' @export
randomize.default <- function(object, ...) {
  do.call(structure,
          c(list(object[sample(length(object))]),
          attributes(object)))
}
